...
" Ultra-minimal color scheme for vim-tiny, which lacks +syntax and
" +termguicolors: 256-color approximation of the tokyonight background/text
set background=dark
highlight clear
...
if 1
  ...
  " Tokyonight, the same theme as the Neovim config. The colorscheme files are
  " generated by folke/tokyonight.nvim and downloaded once into ~/.vim/colors.
  " Swap the style below for tokyonight-storm, -night or -day.
  let s:theme = 'tokyonight-moon'
  let s:colors_dir = expand('~/.vim/colors/')
  if empty(glob(s:colors_dir . s:theme . '.vim')) && executable('curl')
    call mkdir(s:colors_dir, 'p')
    call system('curl -fsSL -o ' . shellescape(s:colors_dir . s:theme . '.vim') .
      \ ' https://raw.githubusercontent.com/folke/tokyonight.nvim/main/extras/vim/colors/'
      \ . s:theme . '.vim')
  endif
  execute 'silent! colorscheme ' . s:theme

  " Fall back to a built-in scheme if the download failed
  if !exists('g:colors_name') || g:colors_name !=# s:theme
    colorscheme habamax
  ...
  " Open NERDTree on the left side of the window
